...
export const formatCurrency = (number = 0, settings) => settings.currency_format.replace(amountPattern, formatNumber(number, settings));
...
export const getCheckoutFieldLabel = ({ label, name }) => label && label.length > 0 ? label : getFieldLabelByKey(name);