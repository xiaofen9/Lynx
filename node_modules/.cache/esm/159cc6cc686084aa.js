"main";_686‍.x([["loadState",()=>loadState]]);let api;_686‍.w("./api",[["default",["api"],function(v){api=v}]]);let queryString;_686‍.w("query-string",[["default",["queryString"],function(v){queryString=v}]]);let getParsedProductFilter,getProductFilterForCategory,getProductFilterForSearch;_686‍.w("../shared/actions",[["getParsedProductFilter",["getParsedProductFilter"],function(v){getParsedProductFilter=v}],["getProductFilterForCategory",["getProductFilterForCategory"],function(v){getProductFilterForCategory=v}],["getProductFilterForSearch",["getProductFilterForSearch"],function(v){getProductFilterForSearch=v}]]);let themeLocales;_686‍.w("./themeLocales",[["*",null,function(v){themeLocales=v}]]);let PAGE,PRODUCT_CATEGORY,PRODUCT,RESERVED,SEARCH;_686‍.w("../shared/pageTypes",[["PAGE",["PAGE"],function(v){PAGE=v}],["PRODUCT_CATEGORY",["PRODUCT_CATEGORY"],function(v){PRODUCT_CATEGORY=v}],["PRODUCT",["PRODUCT"],function(v){PRODUCT=v}],["RESERVED",["RESERVED"],function(v){RESERVED=v}],["SEARCH",["SEARCH"],function(v){SEARCH=v}]]);
...
const getCurrentPage = path => {
	return api.sitemap.retrieve({ path: path, enabled: true }).then(sitemapResponse => {
...
const getThemeSettings = () => {
	return api.theme.settings.retrieve().then(({ status, json }) => json).catch(err => ({}));
};

const getAllData = (currentPage, productFilter, cookie) => {
	return Promise.all([api.checkoutFields.list().then(({ status, json }) => json), api.productCategories.list({ enabled: true, fields: CATEGORIES_FIELDS }).then(({ status, json }) => json), api.ajax.cart.retrieve(cookie).then(({ status, json }) => json), getProducts(currentPage, productFilter), getProduct(currentPage), getPage(currentPage), getThemeSettings()]).then(([checkoutFields, categories, cart, products, product, page, themeSettings]) => {
...
const getFilter = (currentPage, urlQuery, settings) => {
	...
	return Promise.all([getCurrentPage(req.path), api.settings.retrieve().then(({ status, json }) => json), themeLocales.getText(language), api.theme.placeholders.list()]).then(([currentPage, settings, themeText, placeholdersResponse]) => {
