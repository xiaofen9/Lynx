...
//        );
    ...
    function wrapEvalArg(ast) {
        printIidToLoc(ast);
        var ret = replaceInExpr(
            instrumentCodeFunName + "(" + astUtil.JALANGI_VAR + ".getConcrete(" + RP + "1), {wrapProgram: false, isEval: true}," + RP + "2,"
                //TODO added by CAS
                + astUtil.JALANGI_VAR + ".evalInvoked(" + RP + "1, " + RP + "2)).code",
            ast,
            getIid()
        );
        transferLoc(ret, ast);
...
//        var newAst = esprima.parse(code, {loc:true, range:true});
        var newAst = acorn.parse(code, { locations:true, allowHashBang:true, ecmaVersion:6, allowImportExportEverywhere:true });
